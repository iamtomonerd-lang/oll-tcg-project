...
// 《ソウルマジック》を最初から選んでおくか。
//
// ふつうにコストを払えない場面でも「ふつうに払う」側で開いていたため、
// 決定が灰色のまま、なぜ押せないのかも分からなかった。
// 撃てるほうを既定にする。チェックを外せばふつうに払う側へ戻せる。
const ソウルマジックを既定にするか = card =>
  !!card.ソウルマジックで使えるか && !card.支払可能 && !card.場のコアも使えば支払えるか;

// チェックの状態に合わせて、支払い元の指定を選び直す。
// ソウルコアは1個しかないので、どこにあっても自動で選ぶ。
function ソウルマジックの指定を選び直す() {
  支払いの内訳.コスト = [];
  if (!el('paySoulMagicCheck').checked) return;
  const 居場所 = ソウルコアの居場所();
  if (居場所) {
...
function openPayPanel(card) {
  ...
  // 《ソウルマジック》。ふつうに払えない場面では最初から選んでおく。
  el('paySoulMagic').hidden = !card.ソウルマジックで使えるか;
  el('paySoulMagicCheck').checked = ソウルマジックを既定にするか(card);
  ソウルマジックの指定を選び直す();
