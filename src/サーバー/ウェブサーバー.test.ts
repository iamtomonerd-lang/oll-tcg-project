...
test('APIを通した自動対戦', async t => {
  ...
  await t.test('自動対戦が主要な操作を実際に通っている', async () => {
    ...
    // 『起動』はここに含めていない。デッキにはカゼキリ（【起動：フラッシュ】持ち）が
    // 入っているが、その効果は『アタック中』を要求する。今のサーバーはアタック宣言と
    // バトル解決を1回のAPI呼び出しで済ませてしまうため、アタック中の状態が外から見える
    // 瞬間が無く、撃てる場面に入れない。割り込みの窓（フラッシュタイミング）を
    // 作ったら、ここに '起動' を足すこと。
    for (const 操作 of ['召喚', '配置', '使用', 'アタック', 'ブロック', '選択']) {
      assert.ok(
        (合計[操作] ?? 0) > 0,
