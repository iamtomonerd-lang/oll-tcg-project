...
test('［フラッシュ］しか持たないマジックも、メインステップで使えて効果が起きる', async () => {
  ...
  try {
    ...
    //
    // 種を1つに決め打ちしていたら、AIを強くしたときにその場面へ入れなくなって落ちた。
    // 確かめたいのは「その場面が来たら撃てること」なので、
    // 場面が現れるまで種を変えて探す。
    let 撃った = false;
    for (const 種 of [11, 3, 7, 77, 5, 23, 101, 202]) {
      if (撃った) break;
      状態 = (await 叩く('POST', '/api/game/start', { mode: 'vsAI', deck: 'flame', seed: 種 })).state;
      撃った = await この種で撃てるか(状態);
    }
  ...
  // 1つの種を最後まで進め、メインステップで嵐を撃てる場面が来たら撃って確かめる。
  async function この種で撃てるか(状態: any): Promise<boolean> {
    for (let 手 = 0; 手 < 400; 手++) {
      if (状態.試合終了か) break;
      const 視点 = 状態.自分.識別子;
