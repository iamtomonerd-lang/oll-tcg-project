...
async function 自動で対戦する(
  ...
  for (; 手数 < 最大手数; 手数++) {
    if (状態.試合終了か) {
      return { 手数, 決着した: true, 選択待ちに遭遇した回数: 選択待ち回数, 起動効果を撃った回数: 起動回数 };
    }
  ...
  return { 手数, 決着した: false, 選択待ちに遭遇した回数: 選択待ち回数, 起動効果を撃った回数: 起動回数 };
}
...
async function 次の1手を送る(
  ...
  // 2. ブロック判断
  if (状態.保留中のブロック) {
    const 候補 = 状態.自分.フィールド.filter((c: any) => c.表示形式 === '回復');
    const ブロッカー =
  ...
  // 3. 起動効果はときどき撃つ
  if (状態.発動できる起動効果.length > 0 && 乱数() < 0.5) {
  ...
  if (召喚する確率 > 0) {
    ...
    if (出せる手札.length > 0 && 乱数() < 召喚する確率) {
      ...
      return {
        種別: '召喚',
        結果: await 叩く('POST', '/api/action/summon', { as: 視点, cardId: カード.識別子 }),
      };
    }
  }
...
test('APIを通した自動対戦', async t => {
  ...
  await t.test('自動対戦が効果の選択待ちを実際に通っている', async () => {
    // 約束事を検査していても、その場面を一度も通らなければ意味がない。
    // 選択待ちに実際に遭遇していることを確かめる。
    let 合計 = 0;
    for (const 種 of [11, 22, 33, 44, 55, 66]) {
      const 結果 = await 自動で対戦する('vsHuman', 'effect', 種);
      合計 += 結果.選択待ちに遭遇した回数;
    }
    assert.ok(合計 > 0, '一度も選択待ちを通らなかった（テストが素通りしている）');
  });
