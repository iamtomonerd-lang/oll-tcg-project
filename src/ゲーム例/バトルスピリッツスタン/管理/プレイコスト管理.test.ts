...
// === ソウルコアを狙って払う（コア支払い元.ソウルコア数） ===
//
// 名指しできないと、通常コアが余っているかぎりソウルコアは絶対にトラッシュへ行かない。
// そのため「トラッシュの{ソウルコア}をそれに置く」（キュペルLv2）を試す手立てが無かった。

test('コストを支払う：ソウルコア数を名指しすると、通常コアが足りていてもソウルコアを出す', () => {
  const 管理 = new プレイコスト管理();
  const 元 = new テスト用コア保持者(5, true); // 通常4＋ソウルコア1
  const トラッシュ = new テスト用コア保持者(0);

  const 成功 = 管理.コストを支払う([{ 保持者: 元, 数: 3, ソウルコア数: 1 }], トラッシュ, 3);

  assert.equal(成功, true);
  assert.equal(元.ソウルコアあるか(), false, 'ソウルコアは元から出て行く');
  assert.equal(トラッシュ.ソウルコアあるか(), true, 'ソウルコアがトラッシュに入る');
  assert.equal(トラッシュ.コア数を取得(), 3);
  assert.equal(元.コア数を取得(), 2);
});

test('コストを支払う：ソウルコア数に0を名指しすると、足りていてもソウルコアを温存する', () => {
  const 管理 = new プレイコスト管理();
  const 元 = new テスト用コア保持者(3, true); // 通常2＋ソウルコア1
  const トラッシュ = new テスト用コア保持者(0);

  // 通常コアは2個しかないので、名指しが無ければ3個目にソウルコアが出てしまう
  const 成功 = 管理.コストを支払う([{ 保持者: 元, 数: 3, ソウルコア数: 0 }], トラッシュ, 3);

  assert.equal(成功, false, '通常コアだけでは払えないので失敗する');
  assert.equal(元.ソウルコアあるか(), true, 'ソウルコアは動かない');
  assert.equal(トラッシュ.コア数を取得(), 0);
});

test('コストを支払う：ソウルコア数を名指ししなければ、これまで通り通常コアを優先する', () => {
  const 管理 = new プレイコスト管理();
  const 元 = new テスト用コア保持者(5, true);
  const トラッシュ = new テスト用コア保持者(0);

  const 成功 = 管理.コストを支払う([{ 保持者: 元, 数: 3 }], トラッシュ, 3);

  assert.equal(成功, true);
  assert.equal(元.ソウルコアあるか(), true, '通常コアで足りるのでソウルコアは残る');
  assert.equal(トラッシュ.ソウルコアあるか(), false);
});

test('コストを支払う：ソウルコアが無いのに名指しすると失敗する', () => {
  const 管理 = new プレイコスト管理();
  const 元 = new テスト用コア保持者(5, false);
  const トラッシュ = new テスト用コア保持者(0);
...
test('初期コアを配置する：ソウルコア数を名指しして、カードにソウルコアを乗せられる', () => {
  const 管理 = new プレイコスト管理();
  const 元 = new テスト用コア保持者(5, true);
  const 対象 = new テスト用コア保持者(0);

  const 成功 = 管理.初期コアを配置する([{ 保持者: 元, 数: 1, ソウルコア数: 1 }], 対象, 1);

  assert.equal(成功, true);
  assert.equal(対象.ソウルコアあるか(), true);
  assert.equal(対象.コア数を取得(), 1);
});
